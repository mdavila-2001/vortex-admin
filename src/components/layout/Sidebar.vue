...
.vortex-sidebar {
  ...
  background-color: #0f172a; /* sidebar-blue */
  border-right: 1px solid rgba(255, 255, 255, 0.05); /* border-slate-800/50 */
  z-index: 20;
  transition: all 0.3s;
  height: 100vh;
}
...
.nav-menu {
  flex: 1;
  overflow-y: auto;
  padding: 1.5rem 0.75rem; /* py-6 px-3 */
  display: flex;
  flex-direction: column;
  gap: 0.25rem; /* gap-1 */
}
...
.nav-icon { font-size: 1.375rem; /* 22px */ }
.nav-label { font-size: 0.875rem; font-weight: 500; }

.nav-item:hover {
  color: white;
  background-color: rgba(30, 41, 59, 0.5); /* hover:bg-slate-800/50 */
}

/* Estado Activo (Vue Router inyecta esta clase automáticamente) */
.router-link-active {
  background-color: rgba(25, 120, 229, 0.1); /* bg-primary/10 */
